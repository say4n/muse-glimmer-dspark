...
config_path=config/dspark/dspark_muse_glimmer_30b.py
...
train_data_path=train_datasets/muse_glimmer_30b/perfectblend_train_regen.jsonl
...
echo "Step 2/3: generating Muse-Glimmer-30B train data: ${train_data_path}"
echo "Start sglang first with: bash scripts/data/launch_sglang_server.sh"
# Muse Glimmer best practices: temperature=1.0, top_p=0.95, top_k=64.
# Keep temperature=0.7 for deterministic-enough drafting targets (matches DeepSpec).
...
echo "Step 3/3: preparing Muse-Glimmer-30B target cache: ${cache_dir}"
