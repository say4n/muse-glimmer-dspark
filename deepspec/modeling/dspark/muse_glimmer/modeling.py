...
def apply_rotary_pos_emb(q, k, cos, sin, unsqueeze_dim=1):
    """RoPE where q consumes only the trailing (draft) positions of cos/sin.

    The DSpark draft block is a continuation of the context, so q_len can be
    shorter than the full (context + draft) sequence that k spans. This is the
    standard sliced-RoPE variant used by the other DSpark model families.
    """
...
class MuseGlimmerDSparkAttention(nn.Module):
    """Muse Glimmer gated attention adapted for DSpark block drafting.

    Queries come from the draft block; keys/values are the concatenation of the
    target context hidden states (from ``target_hidden_states``) and the draft
    block hidden states (``hidden_states``). Mirrors the target's gated
    attention: QK-norm + ``qk_scale_factor`` and a gated output projection.
    """
    ...
    def __init__(self, config, layer_idx: int):
        ...
        # Weight-less QK-norm, exactly like the target text attention.
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_ctx = self.k_proj(target_hidden_states)
        key_noise = self.k_proj(hidden_states)
        value_ctx = self.v_proj(target_hidden_states)
...
class MuseGlimmerDSparkDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = MuseGlimmerDSparkAttention(config=config, layer_idx=layer_idx)
        self.mlp = MuseGlimmerTextMLP(config)
        self.input_layernorm = MuseGlimmerTextCenteredRMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
        )
        self.post_attention_layernorm = MuseGlimmerTextCenteredRMSNorm(
            config.hidden_size,
            eps=config.post_norm_eps,
        )
        self.pre_feedforward_layernorm = MuseGlimmerTextCenteredRMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
        )
        self.post_feedforward_layernorm = MuseGlimmerTextCenteredRMSNorm(
            config.hidden_size,
            eps=config.post_norm_eps,
        )
...
class MuseGlimmerDSparkModel(MuseGlimmerPreTrainedModel):
    config_class = MuseGlimmerTextConfig
    base_model_prefix = "model"
    _no_split_modules = ["MuseGlimmerDSparkDecoderLayer"]
    ...
    def __init__(self, config) -> None:
        ...
        self.layers = nn.ModuleList(
            [
                MuseGlimmerDSparkDecoderLayer(config, layer_idx)
        ...
        self.hidden_norm = MuseGlimmerRMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
        )
